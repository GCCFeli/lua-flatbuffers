...
private:
	...
	void encodeValue(lua_State* L, Writer* writer, int idx, int depth = 0)
	{
		...
		case LUA_TNUMBER:
			if (luax::isinteger(L, idx, &integer))
				...
				if (!writer->LuaNumber(lua_tonumber(L, idx)))
					luaL_error(L, "error while encode double value.");
			}
			return;
		case LUA_TVECTOR2:
			encodeVector2(writer, lua_tovector2(L, idx));
			return;
		case LUA_TSTRING:
	...
	template<typename Writer>
	void encodeVector2(Writer* writer, const lua_Vector2& v2)
	{
		writer->StartObject();
		writer->Key("x");
		writer->LuaNumber(v2.x);
		writer->Key("y");
		writer->LuaNumber(v2.y);
		writer->EndObject();
	}
