...
constexpr FP_INT Shift = 32;
constexpr FP_LONG FractionMask = (INT64_C(1) << Shift) - 1;  // Space before INT64_C(1) needed because of hacky C++ code generator
constexpr FP_LONG IntegerMask = ~FractionMask;

// Constants
constexpr FP_LONG Zero = INT64_C(0);
constexpr FP_LONG Neg1 = INT64_C(0xFFFFFFFFFFFFFFFF) << Shift;
constexpr FP_LONG One = INT64_C(1) << Shift;
constexpr FP_LONG Two = INT64_C(2) << Shift;
constexpr FP_LONG Three = INT64_C(3) << Shift;
constexpr FP_LONG Four = INT64_C(4) << Shift;
constexpr FP_LONG Half = One >> 1;
constexpr FP_LONG Pi = INT64_C(13493037705);  //(FP_LONG)(Math.PI * 65536.0) << 16;
constexpr FP_LONG Pi2 = INT64_C(26986075409);
constexpr FP_LONG PiHalf = INT64_C(6746518852);
constexpr FP_LONG E = INT64_C(11674931555);

constexpr FP_LONG MinValue = INT64_MIN;
constexpr FP_LONG MaxValue = INT64_C(9223372036854775807);

constexpr FP_LONG MinIntValue = (MinValue >> 32);
constexpr FP_LONG MaxIntValue = (MaxValue >> 32);

// Private constants
constexpr FP_LONG RCP_LN2 = INT64_C(0x171547652);    // 1.0 / log(2.0) ~= 1.4426950408889634
constexpr FP_LONG RCP_LOG2_E = INT64_C(2977044471);  // 1.0 / log2(e) ~= 0.6931471805599453
constexpr FP_INT RCP_HALF_PI = 683565276;            // 1.0 / (4.0 * 0.5 * Math.PI);  // the 4.0 factor converts directly to s2.30

constexpr FP_LONG RCP_EPS = (4295);  // 1e-6

// Converts an integer to a fixed-point value.
FP_INLINE FP_LONG FromInt(FP_INT v) {
    return (FP_LONG)v << Shift;
}

// Converts a double to a fixed-point value.
FP_INLINE FP_LONG FromDouble(double v) {
    return (FP_LONG)(v * 4294967296.0);
}

// Converts a float to a fixed-point value.
FP_INLINE FP_LONG FromFloat(float v) {
    return (FP_LONG)(v * 4294967296.0f);
}

// Converts a fixed-point value into an integer by rounding it up to nearest integer.
FP_INLINE FP_INT CeilToInt(FP_LONG v) {
    return (FP_INT)((v + (One - 1)) >> Shift);
}

// Converts a fixed-point value into an integer by rounding it down to nearest integer.
FP_INLINE FP_INT FloorToInt(FP_LONG v) {
    return (FP_INT)(v >> Shift);
}

// Converts a fixed-point value into an integer by rounding it to nearest integer.
FP_INLINE FP_INT RoundToInt(FP_LONG v) {
    return (FP_INT)((v + Half) >> Shift);
}

// Converts a fixed-point value into a double.
FP_INLINE double ToDouble(FP_LONG v) {
    return (double)v * (1.0 / 4294967296.0);
}

// Converts a FP value into a float.
FP_INLINE float ToFloat(FP_LONG v) {
    return (float)v * (1.0f / 4294967296.0f);
}

// Returns the absolute (positive) value of x.
FP_INLINE FP_LONG Abs(FP_LONG x) {
...
FP_INLINE FP_LONG Nabs(FP_LONG x) {
    return (x > 0) ? -x : x;
}

// Round up to nearest integer.
FP_INLINE FP_LONG Ceil(FP_LONG x) {
    return (x + FractionMask) & IntegerMask;
}

// Round down to nearest integer.
FP_INLINE FP_LONG Floor(FP_LONG x) {
    return x & IntegerMask;
}

// Round to nearest integer.
FP_INLINE FP_LONG Round(FP_LONG x) {
    return (x + Half) & IntegerMask;
}

// Returns the fractional part of x. Equal to 'x - floor(x)'.
FP_INLINE FP_LONG Fract(FP_LONG x) {
    return x & FractionMask;
}

// Returns the minimum of the two values.
FP_INLINE FP_LONG Min(FP_LONG a, FP_LONG b) {
    return (a < b) ? a : b;
}

// Returns the maximum of the two values.
FP_INLINE FP_LONG Max(FP_LONG a, FP_LONG b) {
    return (a > b) ? a : b;
}

// Returns the sign of the value (-1 if negative, 0 if zero, 1 if positive).
FP_INLINE FP_LONG Sign(FP_LONG x) {
    if (x == 0)
        return 0;
    return (x < 0) ? Neg1 : One;
}
...
FP_INLINE FP_LONG Add(FP_LONG a, FP_LONG b) {
    return a + b;
}

// Subtracts the two FP numbers from each other.
FP_INLINE FP_LONG Sub(FP_LONG a, FP_LONG b) {
    return a - b;
}

// Multiplies two FP values together.
FP_INLINE FP_LONG Mul(FP_LONG a, FP_LONG b) {
    FP_LONG ai = a >> Shift;
...
FP_INLINE FP_INT MulIntLongLow(FP_INT a, FP_LONG b) {
    FP_ASSERT(a >= 0);
...
FP_INLINE FP_LONG MulIntLongLong(FP_INT a, FP_LONG b) {
    FP_ASSERT(a >= 0);
    FP_LONG bi = b >> Shift;
    FP_LONG bf = b & FractionMask;
...
FP_INLINE FP_INT Nlz(FP_ULONG x) {
    FP_INT n = 0;
    if (x <= INT64_C(0x00000000FFFFFFFF)) {
        n = n + 32;
        x = x << 32;
    }
    if (x <= INT64_C(0x0000FFFFFFFFFFFF)) {
        n = n + 16;
        x = x << 16;
    }
    if (x <= INT64_C(0x00FFFFFFFFFFFFFF)) {
        n = n + 8;
        x = x << 8;
    }
    if (x <= INT64_C(0x0FFFFFFFFFFFFFFF)) {
        n = n + 4;
        x = x << 4;
    }
    if (x <= INT64_C(0x3FFFFFFFFFFFFFFF)) {
        n = n + 2;
        x = x << 2;
    }
    if (x <= INT64_C(0x7FFFFFFFFFFFFFFF)) {
        n = n + 1;
    }
    if (x == 0)
        return 64;
...
FP_INLINE FP_LONG DivPrecise(FP_LONG arg_a, FP_LONG arg_b) {
    // From http://www.hackersdelight.org/hdcodetxt/divlu.c.txt

    FP_LONG sign_dif = arg_a ^ arg_b;

    constexpr FP_ULONG b = INT64_C(0x100000000);  // Number base (32 bits)
...
FP_INLINE FP_LONG Div(FP_LONG a, FP_LONG b) {
    if (b == MinValue)
        return 0;
...
// Calculates division approximation.
FP_INLINE FP_LONG DivFast(FP_LONG a, FP_LONG b) {
    if (b == MinValue)
        return 0;
...
// Calculates division approximation.
FP_INLINE FP_LONG DivFastest(FP_LONG a, FP_LONG b) {
    if (b == MinValue)
        return 0;
...
FP_INLINE FP_LONG Mod(FP_LONG a, FP_LONG b) {
    FP_LONG di = a / b;
    FP_LONG ret = a - (di * b);
    return ret;
}
...
FP_INLINE FP_LONG SqrtPrecise(FP_LONG a) {
...
FP_INLINE FP_LONG Sqrt(FP_LONG x) {
...
FP_INLINE FP_LONG SqrtFast(FP_LONG x) {
    // Return 0 for all non-positive values.
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG SqrtFastest(FP_LONG x) {
    // Return 0 for all non-positive values.
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG RSqrt(FP_LONG x) {
...
// Calculates the reciprocal square root.
FP_INLINE FP_LONG RSqrtFast(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
// Calculates the reciprocal square root.
FP_INLINE FP_LONG RSqrtFastest(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG Rcp(FP_LONG x) {
    if (x == MinValue || x == 0)
        return 0;
...
// Calculates reciprocal approximation.
FP_INLINE FP_LONG RcpFast(FP_LONG x) {
    if (x == MinValue || x == 0)
        return 0;
...
// Calculates reciprocal approximation.
FP_INLINE FP_LONG RcpFastest(FP_LONG x) {
    if (x == MinValue || x == 0)
        return 0;
...
FP_INLINE FP_LONG Exp2(FP_LONG x) {
...
// Calculates the base 2 exponent.
FP_INLINE FP_LONG Exp2Fast(FP_LONG x) {
    // Handle values that would under or overflow.
    if (x >= 32 * One)
        return MaxValue;
    if (x <= -32 * One)
...
// Calculates the base 2 exponent.
FP_INLINE FP_LONG Exp2Fastest(FP_LONG x) {
    // Handle values that would under or overflow.
    if (x >= 32 * One)
        return MaxValue;
    if (x <= -32 * One)
...
FP_INLINE FP_LONG Exp(FP_LONG x) {
    // e^x == 2^(x / ln(2))
    return Exp2(Mul(x, RCP_LN2));
}

FP_INLINE FP_LONG ExpFast(FP_LONG x) {
    // e^x == 2^(x / ln(2))
    return Exp2Fast(Mul(x, RCP_LN2));
}

FP_INLINE FP_LONG ExpFastest(FP_LONG x) {
    // e^x == 2^(x / ln(2))
    return Exp2Fastest(Mul(x, RCP_LN2));
}

// Natural logarithm (base e).
FP_INLINE FP_LONG Log(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG LogFast(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG LogFastest(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG Log2(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG Log2Fast(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG Log2Fastest(FP_LONG x) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG Pow(FP_LONG x, FP_LONG exponent) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
// Calculates x to the power of the exponent.
FP_INLINE FP_LONG PowFast(FP_LONG x, FP_LONG exponent) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
// Calculates x to the power of the exponent.
FP_INLINE FP_LONG PowFastest(FP_LONG x, FP_LONG exponent) {
    // Return 0 for invalid values
    if (x <= 0)
        return 0;
...
FP_INLINE FP_LONG Sin(FP_LONG x) {
...
FP_INLINE FP_LONG SinFast(FP_LONG x) {
    // Map [0, 2pi] to [0, 4] (as s2.30).
    // This also wraps the values into one period.
    FP_INT z = MulIntLongLow(RCP_HALF_PI, x);
...
FP_INLINE FP_LONG SinFastest(FP_LONG x) {
    // Map [0, 2pi] to [0, 4] (as s2.30).
    // This also wraps the values into one period.
    FP_INT z = MulIntLongLow(RCP_HALF_PI, x);
...
FP_INLINE FP_LONG Cos(FP_LONG x) {
    return Sin(x + PiHalf);
}

FP_INLINE FP_LONG CosFast(FP_LONG x) {
    return SinFast(x + PiHalf);
}

FP_INLINE FP_LONG CosFastest(FP_LONG x) {
    return SinFastest(x + PiHalf);
}

FP_INLINE FP_LONG Tan(FP_LONG x) {
    FP_INT z = MulIntLongLow(RCP_HALF_PI, x);
...
FP_INLINE FP_LONG TanFast(FP_LONG x) {
    FP_INT z = MulIntLongLow(RCP_HALF_PI, x);
...
FP_INLINE FP_LONG TanFastest(FP_LONG x) {
    FP_INT z = MulIntLongLow(RCP_HALF_PI, x);
...
FP_INLINE FP_INT Atan2Div(FP_LONG y, FP_LONG x) {
...
FP_INLINE FP_LONG Atan2(FP_LONG y, FP_LONG x) {
...
FP_INLINE FP_INT Atan2DivFast(FP_LONG y, FP_LONG x) {
    FP_ASSERT(y >= 0 && x > 0 && x >= y);
...
FP_INLINE FP_LONG Atan2Fast(FP_LONG y, FP_LONG x) {
    // See: https://www.dsprelated.com/showarticle/1052.php
...
FP_INLINE FP_INT Atan2DivFastest(FP_LONG y, FP_LONG x) {
    FP_ASSERT(y >= 0 && x > 0 && x >= y);
...
FP_INLINE FP_LONG Atan2Fastest(FP_LONG y, FP_LONG x) {
    // See: https://www.dsprelated.com/showarticle/1052.php
...
FP_INLINE FP_LONG Asin(FP_LONG x) {
    // Return 0 for invalid values
    if (x < -One || x > One)
        return 0;
...
FP_INLINE FP_LONG AsinFast(FP_LONG x) {
    // Return 0 for invalid values
    if (x < -One || x > One)
        return 0;
...
FP_INLINE FP_LONG AsinFastest(FP_LONG x) {
    // Return 0 for invalid values
    if (x < -One || x > One)
        return 0;
...
FP_INLINE FP_LONG Acos(FP_LONG x) {
    // Return 0 for invalid values
    if (x < -One || x > One)
        return 0;
...
FP_INLINE FP_LONG AcosFast(FP_LONG x) {
    // Return 0 for invalid values
    if (x < -One || x > One)
        return 0;
...
FP_INLINE FP_LONG AcosFastest(FP_LONG x) {
    // Return 0 for invalid values
    if (x < -One || x > One)
        return 0;
...
FP_INLINE FP_LONG Atan(FP_LONG x) {
    return Atan2(x, One);
}

FP_INLINE FP_LONG AtanFast(FP_LONG x) {
    return Atan2Fast(x, One);
}

FP_INLINE FP_LONG AtanFastest(FP_LONG x) {
    return Atan2Fastest(x, One);
}

FP_INLINE int Cmpf(FP_LONG a, FP_LONG b) {
    if (a > b + RCP_EPS) {
        return 1;
    } else if (a < b - RCP_EPS) {
        return -1;
...
FP_INLINE int SignEps(FP_LONG num) {
    if (num > RCP_EPS) {
        return 1;
    } else if (num < -RCP_EPS) {
        return -1;
...
FP_INLINE FP_LONG Clamp(FP_LONG num, FP_LONG min, FP_LONG max) {
