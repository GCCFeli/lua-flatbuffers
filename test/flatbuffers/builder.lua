...
function m.New(initialSize)
    ...
    {
        finished = false,
        bytes = flatbuffersnative.new_binaryarray(initialSize),
        nested = false,
        head = initialSize,
        minalign = 1,
        vtables = {}
...
function mt:Output(full)
    ...
    if full then
        return self.bytes:Slice(0, #self.bytes)
    else
        return self.bytes:Slice(self.head, #self.bytes)
    end
end
...
function mt:PrependBoolSlot(o, x, d)    self:PrependSlot(Bool, o, (x and 1 or 0), d) end
