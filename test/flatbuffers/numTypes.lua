...
local bunpack = function(fmt, s, pos)
    if type(s) == "string" then
        return string.unpack(fmt, s, pos + 1)
    end
    return string.unpack(fmt, s:Slice(pos, #s))
end
...
function type_mt:EnforceNumber(n)
    ...
    -- error(string.format("Number %d is not in the valid range [%d, %d] of type %s",
    --     n, self.min_value, self.max_value, self.name))
end
...
local uint64_mt =
{
    bytewidth = 8,
    min_value = 0,
    -- out lua implementation can only support max of int64_t, which is 2^32 - 1
    max_value = 2^63-1,
    lua_type = type(1),
    name = "uint64",
    packFmt = "<I8"
...
local function GenerateTypes(listOfTypes)
